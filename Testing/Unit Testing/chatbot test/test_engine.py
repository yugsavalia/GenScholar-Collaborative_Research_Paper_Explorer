...
class EngineFunctionsTestCase(TestCase):
    ...
    def test_add_pdf_to_workspace_index_success(self, mock_parser, mock_prompt, mock_faiss, mock_loader, mock_llm, mock_embeddings):
        ...
        with patch('chatbot.engine.EMBEDDINGS', mock_embeddings_obj), \
             ...
             patch('pdfs.models.PDFFile.save', MagicMock()):
            # Run function
    ...
    def test_normalize_helpers_and_matching(self):
        from chatbot.engine import (
            _normalize_text,
            _match_pdf_title,
            _validate_specific_pdf_request,
            _detect_pdf_from_query,
            _extract_doc_name_from_query,
            _resolve_target_pdf,
        )

        self.assertEqual(_normalize_text("Hello! 123"), "hello 123")
        matched, ratio = _match_pdf_title(self.workspace, "Test PDF")
        self.assertEqual(matched, self.pdf)
        self.assertEqual(ratio, 1.0)
        self.assertIsNotNone(_validate_specific_pdf_request(self.workspace, "Test PDF"))
        self.assertIsNone(_validate_specific_pdf_request(self.workspace, "unknown"))
        found = _detect_pdf_from_query(self.workspace, "please summarize Test PDF")
        self.assertEqual(found, self.pdf)
        self.assertEqual(_extract_doc_name_from_query("give summary of Test PDF"), "Test")
        resolved = _resolve_target_pdf(self.workspace, None, "summary of Test PDF")
        self.assertEqual(resolved, self.pdf)
    ...
    def test_add_pdf_to_workspace_index_missing_embeddings(self):
        from chatbot.engine import add_pdf_to_workspace_index
        with patch('chatbot.engine.EMBEDDINGS', None), patch('chatbot.engine.LLM', MagicMock()):
            add_pdf_to_workspace_index(self.pdf.id)
        self.workspace.refresh_from_db()
        self.assertEqual(self.workspace.processing_status, Workspace.ProcessingStatus.FAILED)

    @patch('chatbot.engine.PDFPlumberLoader')
    @patch('chatbot.engine.tempfile.NamedTemporaryFile')
    @patch('chatbot.engine.os.remove')
    @patch('chatbot.engine.os.path.exists', return_value=False)
    def test_add_pdf_to_workspace_index_loader_failure(self, mock_exists, mock_remove, mock_tempfile, mock_loader):
        from chatbot.engine import add_pdf_to_workspace_index
        temp_context = MagicMock()
        temp_context.__enter__.return_value.name = '/tmp/test.pdf'
        temp_context.__enter__.return_value.write = MagicMock()
        mock_tempfile.return_value = temp_context

        loader_instance = MagicMock()
        loader_instance.load.side_effect = Exception("PDF load error")
        mock_loader.return_value = loader_instance

        with patch('chatbot.engine.EMBEDDINGS', MagicMock()), patch('chatbot.engine.LLM', MagicMock()):
            add_pdf_to_workspace_index(self.pdf.id)
    ...
    def test_get_chatbot_response_summary_specific_doc_no_match(self, mock_classify):
        ...
        self.assertIn("pdf not available", result.lower())
    ...
    @patch('os.makedirs')
    @patch('os.path.join')
    def test_add_pdf_to_workspace_index_existing_index(self, mock_join, mock_makedirs, mock_exists, mock_splitter, mock_parser, mock_prompt, mock_faiss, mock_loader, mock_llm, mock_embeddings, mock_remove, mock_tempfile):
        ...
        mock_tempcontext = MagicMock()
        mock_tempcontext.__enter__.return_value = mock_temp_file
        mock_tempfile.return_value = mock_tempcontext

        # Mock os.remove to be no-op
        ...
        with patch('chatbot.engine.EMBEDDINGS', mock_embeddings_obj), \
             patch('chatbot.engine.LLM', mock_llm_obj):
            with patch('pdfs.models.PDFFile.save', MagicMock()), \
                 patch('workspaces.models.Workspace.save', MagicMock()):
                add_pdf_to_workspace_index(self.pdf.id)
